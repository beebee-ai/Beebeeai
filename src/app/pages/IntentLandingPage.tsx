...
const pages = {
  ...
  university: {
    ...
    points: ['适合 18 岁以上大学生、研究生、职场人士与工程学习者', '星空营 Galaxy 6 周 · 深空营 Cosmos 8 周 · 无界营 Infinity 12 周', '覆盖 AI 产品设计、全栈工程、RAG、Agent 与市场验证', '项目成果可用于作品集、申请或面试展示'],
  ...
  enterprise: {
    ...
    eyebrow: 'Enterprise AI · 陪跑服务',
    title: '企业 AI 咨询与陪跑落地服务',
    description: '面向企业团队的 AI 咨询与落地服务，围绕知识库、RAG、AI Agent、内容生产和业务流程设计可执行方案。',
    lead: 'BEEBEE AI 是企业 AI 陪跑专家：从第一个 Agent 上线，到 AI 真正进入企业日常业务。连接企业现有系统，持续构建、运营和优化 AI Agent。',
    ...
    cta: '查看 BEE Sigma 平台与案例', href: 'https://beesigma.com/',
  },
  skills: {
    ...
    <main className="pt-16">
      <section className="px-4 pt-16 md:pt-24 pb-14 md:pb-20 border-b border-white/10">
        <div className="max-w-6xl mx-auto">
          <Link to="/" className="text-sm text-gray-500 hover:text-gold-500 transition-colors">← {language === 'ZH' ? '返回首页' : 'Back home'}</Link>
          <p className="mt-10 text-xs font-mono tracking-[0.24em] text-gold-500">{page.eyebrow}</p>
        ...
        <div className="max-w-6xl mx-auto grid lg:grid-cols-[1fr_.9fr] gap-12 lg:gap-20">
          <div>
            ...
            <ul className="border-y border-white/10 divide-y divide-white/10">
              {page.points.map(point => <li key={point} className="py-5 flex gap-4 text-gray-200 leading-7"><CheckCircle2 className="text-gold-500 shrink-0 mt-1" size={19} />{point}</li>)}
            </ul>
            {'internal' in page && page.internal ? <Link to={page.href} className="mt-8 inline-flex items-center gap-2 text-gold-500 hover:text-gold-400">{page.cta}<ArrowRight size={18} /></Link> : <a href={page.href} target="_blank" rel="noopener noreferrer" className="mt-8 inline-flex items-center gap-2 text-gold-500 hover:text-gold-400">{page.cta}<ArrowRight size={18} /></a>}
          </div>
          <div>
