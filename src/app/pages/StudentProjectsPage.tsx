...
export function StudentProjectsPage() {
  ...
  return <div className="min-h-screen bg-[var(--bg)] text-white">
    ...
    <main className="pt-16">
      <section className="px-4 pt-12 md:pt-20 pb-10 md:pb-16 border-b border-white/10">
        <div className="max-w-7xl mx-auto">
          <Link to="/#works" className="text-sm text-gray-500 hover:text-gold-500 transition-colors">← {zh ? '返回首页' : 'Back home'}</Link>
          <div className="mt-10">
            <div>
              <div className="flex items-center gap-3 text-gold-500"><Layers3 size={20} /><span className="text-xs font-mono tracking-[0.28em]">STUDENT PROJECTS</span></div>
              <h1 className="mt-5 max-w-4xl text-4xl md:text-6xl font-semibold leading-[1.12]">{zh ? <>不是练习题，<br /><span className="text-gold-500">是真实可访问的作品</span></> : <>Not exercises.<br /><span className="text-gold-500">Real products you can visit.</span></>}</h1>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
            {visibleProjects.map((project, index) => {
              ...
              return <article key={project.url} className={`group relative overflow-hidden rounded-2xl border border-white/10 bg-[var(--panel)] hover:border-white/25 transition-colors ${featured ? 'md:col-span-2 lg:col-span-2' : ''}`}>
                <div className={featured ? 'lg:grid lg:grid-cols-[1.15fr_.85fr] h-full' : 'h-full flex flex-col'}>
                  <a href={project.url} target="_blank" rel="noopener noreferrer" className={`relative block overflow-hidden bg-[#0e0d0b] ${featured ? 'min-h-[260px] md:min-h-[360px]' : 'aspect-[16/10]'}`}>
                    ...
                    <span className="absolute z-20 top-4 left-4 px-2.5 py-1 rounded-full bg-black/70 backdrop-blur text-[11px] font-mono tracking-wider text-white border border-white/10">{type === 'alpha' ? 'Alpha' : 'Beta'}</span>
                    <span className="absolute z-20 bottom-4 right-4 w-10 h-10 rounded-full bg-gold-500 text-black flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><ArrowUpRight size={19} /></span>
                  </a>
                  <div className={`p-5 md:p-6 flex flex-col ${featured ? 'justify-center' : 'flex-1'}`}>
                    <div className="flex items-start justify-between gap-4">
                      <div><p className={`text-[11px] font-mono tracking-wider ${type === 'alpha' ? 'text-orange-500' : 'text-cyan-400'}`}>{t(project.badge, language)}</p><h2 className={`${featured ? 'text-2xl md:text-3xl' : 'text-xl'} mt-2 font-semibold leading-snug group-hover:text-gold-500 transition-colors`}>{t(project.title, language)}</h2></div>
                    ...
                    <a href={project.url} target="_blank" rel="noopener noreferrer" className="mt-6 pt-4 border-t border-white/10 inline-flex items-center justify-between text-sm text-white hover:text-gold-500 transition-colors">{zh ? '访问项目' : 'Visit project'} <ArrowUpRight size={17} /></a>
                  </div>
                </div>
              </article>;
            })}
